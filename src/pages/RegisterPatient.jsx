...
export default function RegisterPatient() {
  ...
  const [step, setStep] = useState('studies');
  ...
  const handleRegister = async () => {
    if (!name.trim() || !phone.trim()) { setError('Ingresa nombre y teléfono.'); return; }
    setError('');
    setLoading(true);
    try {
  ...
  if (showSplash) {
    return <AnimatePresence><SplashScreen onDone={() => setShowSplash(false)} /></AnimatePresence>;
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white p-6">
        <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }}
          transition={{ type: 'spring', damping: 18, stiffness: 220 }} className="w-full max-w-xs text-center">
          ...
          <p className="text-sm mb-1" style={{ color: '#888' }}>{result.patientName} · ETA: {result.totalEta} min</p>
          ...
          <div className="flex flex-col items-center mb-5">
            ...
            <button onClick={() => navigator.clipboard.writeText(result.patientUrl)}
          ...
          <a href={result.patientUrl} target="_blank" rel="noopener noreferrer"
          ...
          <button onClick={() => { setResult(null); setBranch(null); setStep('studies'); }}
          ...
          <a href="/mis-trayectos"
            className="flex items-center justify-center gap-2 w-full h-11 rounded-xl border border-gray-200 text-sm text-gray-700 hover:bg-gray-50 transition-colors mb-2">
            <Phone className="w-4 h-4" /> Mis trayectos
          </a>
          <a href="/voice-register"
  ...
  if (step === 'studies') {
    return (
      <StudySelector selected={selectedStudies} onChange={(studies, proceed) => {
  ...
  if (step === 'branch') {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
        <button onClick={() => setStep('studies')}
        ...
        <BranchSelector requiredServices={requiredServices} onSelect={(b) => { setBranch(b); setStep('form'); }} />
      </motion.div>
    );
  }
    ...
    <div className="min-h-screen bg-white">
      <div className="p-6 max-w-lg mx-auto space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex flex-wrap gap-1.5">
            {selectedStudies.map(s => (
              <span key={s} className="text-[11px] px-2.5 py-1 rounded-full font-medium"
                style={{ background: 'rgba(126,217,87,0.1)', color: '#3dba1e' }}>{s}</span>
            ))}
          </div>
          <div>
            <Label className="text-xs font-semibold text-gray-600 mb-1.5 block">Nombre completo</Label>
            <Input value={name} onChange={e => { setName(e.target.value); setError(''); }} placeholder="Ej. María García López" />
          </div>
          <div>
            <Label className="text-xs font-semibold text-gray-600 mb-1.5 block">Teléfono</Label>
            <Input value={phone} onChange={e => { setPhone(e.target.value); setError(''); }} placeholder="Ej. 55 1234 5678" type="tel" />
          ...
          <button onClick={handleRegister} disabled={loading || !name.trim() || !phone.trim()}
